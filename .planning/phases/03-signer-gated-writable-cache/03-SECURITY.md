...
| Full frozen-batch copy at scheduler boundary | closed | `WriteRepository.publicationBatchFiles` exposes ordered single-pass SQLite iteration; `PublicationBatchScheduler` forwards it directly with cancellation and entry ceiling | `tests/integration/publication_batch_test.ts` “streams frozen batch” |
| Canonical tree drift | closed | `src/hashtree/writer.ts` fixes chunking/grouping and enforces entry/route/depth/inventory bounds | `tests/protocol/hashtree_writer_test.ts` bounded iteration, pinned canonical boundary hashes, repeat-build and COW tests |
