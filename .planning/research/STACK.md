...
**Domain:** Deno/TypeScript daemon bridging Nostr, Blossom Hashtrees, and the Nix HTTP binary-cache protocol
**Researched:** 2026-08-12
**Confidence:** MEDIUM-HIGH (HIGH for released runtime/package facts; MEDIUM for proposed Hashtree BUD implementation choices)
...
No released package currently implements the exact BUD-15/16/17/18 proposal set and the stricter `NIP.md` profile. Implement these as small, independently tested modules rather than adopting an unpublished git branch:
...
Use Deno's package commands so `deno.json` and `deno.lock` remain the source of truth:
...
deno add npm:applesauce-sqlite@6.0.0

# Test dependencies
deno add --dev jsr:@std/assert@1.0.19 \
  jsr:@std/testing@1.0.20 \
  npm:fast-check@4.9.0
```

Check in `deno.lock`. Pin Deno `2.9.5` in the development shell/container and CI rather than relying on whichever Deno happens to be installed.

## Prescriptive Integration Pattern

```text
applesauce-relay Observable<NostrEvent | EOSE>
  -> cryptographic + NIP-XX validation
  -> EventStore.add(validEvent)
  -> custom CachePublication cast/model per configured identity
  -> combineLatest(priority-ordered identities, signer state)
  -> immutable merged-index snapshot
  -> Deno.serve request handler

HTTP/Blossom bytes:
ReadableStream<Uint8Array>
  -> byte-limit TransformStream
  -> incremental hash TransformStream
  -> staging file / verified response
```

Use RxJS to describe lifecycle and state transitions. Use Web Streams for every byte path. Cross the boundary only with small metadata such as a verified hash, staged-file handle, manifest record, or publication result.

For the five-second quiet/sixty-second maximum publication rule, compose a publication intent stream from committed PUT transactions. A quiet timer and a max-window timer may race, but the actual publisher must be serialized (`exhaustMap` or a single-flight queue) and must snapshot the committed pending generation transactionally. A timer firing must never publish an upload that has not finished and been verified.
...
**Read-only daemon (default until signer connected):**
...
**Protected local-key writable daemon:**
...
**Self-encrypted Hashtree enabled:**
...
---
*Stack research for: nixstr-cache*
*Researched: 2026-08-12*
