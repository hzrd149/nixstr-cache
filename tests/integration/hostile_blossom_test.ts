...
import { PathResolver, RequestBudget, VerifiedAbsent } from "../../src/hashtree/reader.ts";
...
Deno.test("traversal|backpressure|HEAD: lazy lookup ignores unrelated missing branches", async () => {
  ...
  const root = manifest({ l: [
    { h: contentHash, n: "wanted", s: content.length, t: 0 },
    { h: new Uint8Array(32).fill(9), n: "zzz", s: 0, t: 2 },
  ], t: 2 });
  const blobs = new Map([[hex(hashBytes(root)), root], [hex(contentHash), content]]);
  const calls: string[] = [];
  const spool = await Deno.makeTempDir();
  const blobFetcher = new BlobFetcher({ fetcher: { fetch: (url: string | URL) => { const h = String(url).split("/").at(-1)!; calls.push(h); const b = blobs.get(h); return Promise.resolve(b ? response(b) : response(new Uint8Array(), 404)); } }, quarantine: { isQuarantined: () => false, quarantine: () => {}, releaseQuarantine: () => {} }, spoolDirectory: spool });
  const resolver = new PathResolver(blobFetcher, buildSourcePlan({ event: ["http://tree.test"] }), { maxWireBytes: 4096, maxDecodedBytes: 4096, maxLinks: 10 });
  const result = await resolver.resolve(hex(hashBytes(root)), "wanted", "GET", new RequestBudget({ maxDepth: 5, maxLinks: 20, maxUniqueNodes: 10, maxDecodedBytes: 10000, maxAttempts: 10, maxRedirects: 3, maxConcurrent: 2, deadline: Date.now() + 5000 }));
...
Deno.test("traversal|backpressure|HEAD: authenticates final link without fetching content", async () => {
  ...
  const root = manifest({ l: [{ h: hashBytes(content), n: "file", s: content.length, t: 0 }], t: 2 });
  let calls = 0;
  const blobFetcher = new BlobFetcher({ fetcher: { fetch: () => { calls++; return Promise.resolve(response(root)); } }, quarantine: { isQuarantined: () => false, quarantine: () => {}, releaseQuarantine: () => {} }, spoolDirectory: await Deno.makeTempDir() });
  const resolver = new PathResolver(blobFetcher, buildSourcePlan({ event: ["http://tree.test"] }), { maxWireBytes: 4096, maxDecodedBytes: 4096, maxLinks: 10 });
  const result = await resolver.resolve(hex(hashBytes(root)), "file", "HEAD", new RequestBudget({ maxDepth: 5, maxLinks: 20, maxUniqueNodes: 10, maxDecodedBytes: 10000, maxAttempts: 10, maxRedirects: 3, maxConcurrent: 2, deadline: Date.now() + 5000 }));
...
Deno.test("traversal|backpressure|HEAD: absence and budget overflow remain typed", async () => {
  ...
  const blobFetcher = new BlobFetcher({ fetcher: { fetch: () => Promise.resolve(response(root)) }, quarantine: { isQuarantined: () => false, quarantine: () => {}, releaseQuarantine: () => {} }, spoolDirectory: await Deno.makeTempDir() });
  const resolver = new PathResolver(blobFetcher, buildSourcePlan({ event: ["http://tree.test"] }), { maxWireBytes: 4096, maxDecodedBytes: 4096, maxLinks: 10 });
  await assertRejects(() => resolver.resolve(hex(hashBytes(root)), "missing", "HEAD", new RequestBudget({ maxDepth: 5, maxLinks: 20, maxUniqueNodes: 10, maxDecodedBytes: 10000, maxAttempts: 10, maxRedirects: 3, maxConcurrent: 2, deadline: Date.now() + 5000 })), VerifiedAbsent);
  const budget = new RequestBudget({ maxDepth: 1, maxLinks: 1, maxUniqueNodes: 1, maxDecodedBytes: 1, maxAttempts: 1, maxRedirects: 1, maxConcurrent: 1, deadline: Date.now() + 5000 });
  budget.debitLinks(1);
